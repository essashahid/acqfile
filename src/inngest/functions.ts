...
function translate(err: unknown, attempt: number): never {
  if (err instanceof StepFailure && err.retryable) throw new RetryAfterError(err.message, RETRY_SCHEDULE_MS[Math.min(attempt, RETRY_SCHEDULE_MS.length - 1)] ?? 2000);
...
export const processDealRunFn = inngest.createFunction({ id: "deal-run", triggers: [dealRunRequested], retries: Math.min(RETRY_SCHEDULE_MS.length, 20) as 3, concurrency: { limit: 2 } }, async ({ event, attempt }) => {
  const [user] = await getDb().select().from(schema.appUsers).where(eq(schema.appUsers.id, event.data.userId));
  ...
  try {
    return await processDealRun({ user: { id: user.id, email: user.email, displayName: user.displayName }, workspace }, event.data.dealId, event.data.runId);
  } catch (err) {
