...
export function normalizeName(v: string) {
  return v
    ...
    .toLowerCase()
    .replace(/\bl\.?l\.?c\.?\b/g, " ")
    .replace(/\b(incorporated|inc|limited|ltd|corporation|corp|llc)\b/g, " ")
    .replace(/[^a-z0-9]/g, "");
}
export function normalizeValue(v: unknown): unknown {
  ...
  if (v && typeof v === "object") {
    ...
    if ("hmac" in o) return o.hmac;
    return Object.fromEntries(
      Object.entries(o)
        .filter(([k]) => k !== "title")
        .map(([k, a]) => [k, normalizeValue(a)]),
    );
  }
  return v;
}
export function equal(a: unknown, b: unknown, tolerance = 0) {
  return typeof a === "number" && typeof b === "number"
    ? Math.abs(a - b) <= tolerance + 1e-9
    : stableStringify(normalizeValue(a)) === stableStringify(normalizeValue(b));
}
export type Resolve = (
  ref: Exclude<Expr, string | number | boolean | null | unknown[] | { op: string; args: Expr[] }>,
) => Value;
export function evaluateExpression(expr: Expr, resolve: Resolve, tolerance = 0): Value {
  if (expr === null || typeof expr !== "object")
    return expr === null || expr === "unknown" ? UNKNOWN : expr;
  ...
  if (args.some((a) => a === UNKNOWN || a === null || a === undefined || a === "unknown"))
    return UNKNOWN;
  const [a, b] = args;
  ...
  switch (expr.op) {
    case "and":
      return args.every((v) => typeof v === "boolean") ? args.every(Boolean) : UNKNOWN;
    case "or":
      return args.every((v) => typeof v === "boolean") ? args.some(Boolean) : UNKNOWN;
    case "not":
      return typeof a === "boolean" ? !a : UNKNOWN;
    case "exists":
    ...
    case "!=":
      return !equal(a, b, tolerance);
    case "in":
      return Array.isArray(b) ? b.some((x) => equal(a, x)) : UNKNOWN;
    case "<":
    case "<=":
    case ">":
    case ">=": {
    ...
    case "+":
      return nums ? (a as number) + (b as number) : UNKNOWN;
    case "-":
      return nums ? (a as number) - (b as number) : UNKNOWN;
    case "*":
      return nums ? (a as number) * (b as number) : UNKNOWN;
    case "/":
      return nums && b !== 0 ? (a as number) / (b as number) : UNKNOWN;
    case "abs":
      return nums ? Math.abs(a as number) : UNKNOWN;
    case "sum":
    case "min":
    case "max": {
      const list = Array.isArray(a) ? a : [a];
      if (
        !list.every((v) => typeof v === "number" && Number.isFinite(v)) ||
        (!list.length && expr.op !== "sum")
      )
        return UNKNOWN;
      return expr.op === "sum"
        ? list.reduce((s: number, v: number) => s + v, 0)
        : expr.op === "min"
          ? Math.min(...list)
          : Math.max(...list);
    }
    case "days_between":
      return date(a) && date(b) ? daysBetween(a, b) : UNKNOWN;
    case "add_days":
      return date(a) && typeof b === "number" && Number.isInteger(b) ? addDays(a, b) : UNKNOWN;
    case "add_years":
      return date(a) && typeof b === "number" && Number.isInteger(b) ? addYears(a, b) : UNKNOWN;
  }
}
