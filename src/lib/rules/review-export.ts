...
export function reviewRows(pack: ResolvedPack): string[][] {
  return [
    ...
    ...[...pack.items, ...pack.consistency].map((r) => [
      pack.version,
      pack.overlay ?? "None",
      r.id,
      r.title,
      `${r.description} ${r.checks.map((c) => c.message).join("; ")}`,
      r.scope,
      r.period_requirement ?? "None",
      String(r.required),
      stableStringify(pack.parameters),
      r.source_ref.class,
      r.source_ref.citation,
      r.source_ref.url ?? "",
      "false",
      "",
    ]),
...
export function exportReview(packs: ResolvedPack[]) {
  ...
  const sections = packs.map((pack, index) => {
    const rows = reviewRows(pack),
      notice = footer(pack);
    const sheet = XLSX.utils.aoa_to_sheet([...rows, [], [notice]]);
    sheet["!cols"] = rows[0]!.map((_, i) => ({
      wch: i === 4 || i === 10 ? 80 : i === 8 ? 45 : 22,
    }));
    XLSX.utils.book_append_sheet(
      book,
      sheet,
      `${index + 1}-${pack.version.slice(4)}${pack.overlay ? "-SLA" : ""}`,
    );
    return `<section><h2>${escape(pack.version)} · ${escape(pack.overlay ?? "Base")}</h2><p>All rules unverified. Content hash: ${pack.content_hash}</p><table><thead><tr>${rows[0]!.map((v) => `<th>${escape(v)}</th>`).join("")}</tr></thead><tbody>${rows
      .slice(1)
      .map((row) => `<tr>${row.map((v) => `<td>${escape(v)}</td>`).join("")}</tr>`)
      .join("")}</tbody></table><footer>${escape(notice)}</footer></section>`;
  });
  return {
