...
function expressions(rule: Rule): Expr[] {
  return [
    rule.applies_when,
    ...rule.checks.flatMap((c) => [c.expr, c.when].filter((e): e is Expr => e !== undefined)),
  ];
}
export function validatePack(input: unknown): Pack {
  ...
  for (const r of rules) {
    if (BANNED_TERMS.test([r.title, r.description, ...r.checks.map((c) => c.message)].join(" ")))
      throw new Error(`Banned term in ${r.id}`);
    if (
      !r.accepts.length &&
      !r.checks.every((c) => c.type === "tracking" || c.expr || c.mode === "party_assignment")
    )
      throw new Error(`${r.id}: evidence types required`);
    const parameter = (key: string) => {
      if (!Object.hasOwn(pack.parameters, key))
        throw new Error(`${r.id}: unknown parameter ${key}`);
    };
    const walk = (e: Expr, depth = 0) => {
      ...
      if (
        "fact" in e &&
        e.types?.some((t) => !(FACTS[e.fact]!.producers as readonly string[]).includes(t))
      )
        throw new Error(`${r.id}: invalid producer for ${e.fact}`);
    };
    expressions(r).forEach((e) => walk(e));
    for (const c of r.checks) {
      for (const key of [c.max_age_param, c.revision_param, c.tolerance_param])
        if (key) parameter(key);
      for (const fact of [...(c.facts ?? []), ...(c.fact ? [c.fact] : [])]) {
        const types = c.across ?? r.accepts;
        if (!types.some((t) => FACTS[fact]!.producers.includes(t)))
          throw new Error(`${r.id}: no producer for ${fact}`);
        if (c.across?.some((t) => !FACTS[fact]!.producers.includes(t)))
          throw new Error(`${r.id}: invalid agreement producer`);
      }
    }
  }
  if (!pack.index.filename_template.endsWith("{original_extension}"))
    throw new Error("Filename must preserve original extension");
  return pack;
}
export function resolvePack(input: unknown, overlayInput?: unknown): ResolvedPack {
  ...
  if (overlayInput) {
    ...
    for (const operation of overlay.operations) {
      if (operation.op === "add") {
        if ([...pack.items, ...pack.consistency].some((r) => r.id === operation.rule.id))
          throw new Error(`Overlay duplicate ${operation.rule.id}`);
        pack[operation.collection].push(operation.rule);
      } else {
        const collection = pack.items.some((r) => r.id === operation.id)
          ? pack.items
          : pack.consistency;
    ...
    for (const key of Object.keys(overlay.parameters))
      if (!Object.hasOwn(pack.parameters, key))
        throw new Error(`Overlay parameter missing: ${key}`);
...
export function loadPack(version: string, overlay?: string): ResolvedPack {
  if (!(PACK_VERSIONS as readonly string[]).includes(version))
    throw new Error("Unknown pack version");
  if (overlay && !(OVERLAY_IDS as readonly string[]).includes(overlay))
    throw new Error("Unknown overlay");
  const root = path.join(process.cwd(), "rulepacks");
  return resolvePack(
    parseYaml(fs.readFileSync(path.join(root, "sba7a-cho", `${version}.yaml`), "utf8")),
    overlay
      ? parseYaml(fs.readFileSync(path.join(root, "overlays", `${overlay}.yaml`), "utf8"))
      : undefined,
  );
}
export function canonicalPack(pack: ResolvedPack) {
  const { content_hash: _hash, ...content } = pack;
  void _hash;
  return stableStringify(content);
}
export function resolvedYaml(pack: ResolvedPack) {
  return stringify(JSON.parse(canonicalPack(pack)), { lineWidth: 120 });
}
    ...
    const used = (rule: Rule | undefined, pack: ResolvedPack) => {
      ...
      return {
        ...
        parameters: Object.fromEntries(
          Object.entries(pack.parameters).filter(([k]) => json.includes(`"${k}"`)),
        ),
      };
    };
    const before = used(l, left),
      after = used(r, right);
    return stableStringify(before) === stableStringify(after)
      ? []
      : [{ id, change: !l ? "added" : !r ? "removed" : "changed", before, after }];
  });
}
