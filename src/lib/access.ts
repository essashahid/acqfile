...
export function mutationAllowed(
  ...
  roles: WorkspaceRole[] = ["admin", "reviewer"],
): boolean {
  return (
    !context.isPublic &&
    roles.includes(context.workspace.role) &&
    (!env().PUBLIC_DEMO_MODE || env().DEMO_MUTATIONS_ENABLED || context.workspace.role === "admin")
  );
}
...
export async function assertMutation(
  ...
  if (!mutationAllowed(context, roles))
    throw new Error(
      "This workspace is read-only. Sign in with an authorized account to make changes.",
    );
  if (["upload", "upload-sign", "reprocess", "run", "eval"].includes(action) && !jobsConfigured())
    throw new Error(
      "Background processing is not configured yet. Connect Inngest to enable this action.",
    );
