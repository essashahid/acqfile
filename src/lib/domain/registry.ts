...
const TaxonomyEntry = z.strictObject({
  id: DocumentTypeSchema,
  extraction: z.boolean(),
  single_instance: z.boolean(),
  label: z.string().min(1),
});
export const TAXONOMY = z.array(TaxonomyEntry).parse(
  DOCUMENT_TYPES.map((id) => ({
    ...
    extraction: (EXTRACTED_TYPES as readonly string[]).includes(id),
...
function register(
  ...
  producers: DocumentType[],
  period_kind: FactDefinition["period_kind"] = "none",
  subject_kind: FactDefinition["subject_kind"] = "party",
...
register("party.identifier", "identifier", [
  "SBA_1919",
  "TAX_PERSONAL",
  "TAX_BUSINESS",
  "PURCHASE_AGREEMENT",
...
for (const a of ["purchase_price", "seller_note_amount"])
  register(`deal.${a}`, "money", ["LOI", "PURCHASE_AGREEMENT", "SOURCES_USES"], "none", "deal");
...
register(
  ...
  "text",
  ["LOI", "PURCHASE_AGREEMENT", "SELLER_NOTE", "SOURCES_USES"],
  "none",
  "deal",
);
...
for (const a of ["sources", "uses"]) {
  register(`funding.${a}`, "amounts", ["SOURCES_USES"], "none", "deal");
  register(`funding.${a}_total`, "money", ["SOURCES_USES"], "none", "deal");
}
for (const a of ["cash", "total_assets", "total_liabilities", "net_worth"])
  register(`pfs.${a}`, "money", ["SBA_413"], "as_of");
...
for (const a of [
  ...
  register(`tax.${a}`, "money", ["TAX_BUSINESS"], "tax_year");
for (const a of ["revenue", "net_income", "total_assets", "total_liabilities"])
  register(`financial.${a}`, "money", ["FIN_YEAR_END", "FIN_INTERIM"], "financial_period");
for (const a of ["period_start", "period_end"])
  register(`financial.${a}`, "date", ["FIN_YEAR_END", "FIN_INTERIM"], "financial_period");
...
export const FactAttributeSchema = z
  .string()
  .refine((a) => Object.hasOwn(FACTS, a), "Unknown fact attribute");
