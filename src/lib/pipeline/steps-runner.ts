...
export function idempotencyKey(ctx: StepContext, stepName: string): string {
  if (ctx.pipelineVersion)
    return `${ctx.workspaceId}:${ctx.documentVersionId}:${stepName}:${ctx.pipelineVersion}:${ctx.modelConfigHash}${ctx.scope ? `:${ctx.scope}` : ""}`;
 ...
 */
async function runStepUnlocked<T>(
  ctx: StepContext,
  stepName: string,
  body: () => Promise<T>,
): Promise<{ output: T; reused: boolean }> {
  const db = getDb();
  ...
  if (existing?.status === "succeeded") {
    if (existing.processingRunId !== ctx.processingRunId) {
      await logEvent(
        ...
        "info",
        "step.reused",
        `${stepName}: reused output from run ${existing.processingRunId}`,
        { stepName, key },
      );
    }
  ...
  if (attempt > 1) {
    await db
      .update(schema.processingRuns)
      .set({ retries: sql`${schema.processingRuns.retries} + 1` })
      .where(eq(schema.processingRuns.id, ctx.processingRunId));
  }
  await logEvent(
    ctx.processingRunId,
  ...
  try {
    if (
      ctx.injectFailure &&
      ctx.injectFailure.step === stepName &&
      attempt <= ctx.injectFailure.attempts
    ) {
      throw new StepFailure(
        `injected failure on ${stepName} attempt ${attempt}`,
        "injected_failure",
        true,
      );
    }
    ...
    await db
      ...
      .where(
        and(
          eq(schema.deadLetters.documentVersionId, ctx.documentVersionId),
          eq(schema.deadLetters.failedStep, stepName),
    ...
    await logEvent(
      ...
      "info",
      "step.succeeded",
      `${stepName}: succeeded in ${Date.now() - started} ms`,
      { stepName, attempt, latencyMs: Date.now() - started },
    );
    return { output, reused: false };
  } catch (err) {
    // Diagnostics stay on the console; persisted run events never carry source text.
    if (process.env.ACQFILE_DEBUG === "1" || process.env.ACQFILE_DEBUG === "true")
      console.error(`[acqfile] ${stepName} failed:`, err);
    const failure =
      err instanceof StepFailure
        ? err
        : new StepFailure(
            ctx.pipelineVersion
              ? "Deal processing step failed; retry or review the source."
              : err instanceof Error
                ? err.message
                : String(err),
            (err as { code?: string })?.code ?? "step_error",
            isRetryable(err),
          );
    ...
    await logEvent(
      ...
      "error",
      "step.failed",
      `${stepName}: ${failure.message}`,
      { stepName, attempt, code: failure.code, retryable: failure.retryable, exhausted },
    );
    if (exhausted) {
      ...
      const versionExists =
        ...
        (
          await db
            ...
            .where(eq(schema.documentVersions.id, ctx.documentVersionId))
            .limit(1)
        ).length > 0;
      if (versionExists)
...
export async function reopenStep(
  ...
  await db
    ...
    .where(
      and(
        eq(schema.runSteps.processingRunId, processingRunId),
        eq(schema.runSteps.documentVersionId, documentVersionId),
        eq(schema.runSteps.stepName, stepName),
      ),
    );
  await db
    .update(schema.deadLetters)
    ...
    .where(
      and(
        eq(schema.deadLetters.processingRunId, processingRunId),
        eq(schema.deadLetters.documentVersionId, documentVersionId),
        eq(schema.deadLetters.failedStep, stepName),
      ),
    );
}
