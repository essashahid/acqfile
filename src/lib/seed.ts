...
export async function seedWorkspace(): Promise<SeedResult> {
  ...
  let [ws] = await db
    ...
    .where(eq(schema.workspaces.slug, "default"))
    .limit(1);
  if (!ws)
    [ws] = await db
      .insert(schema.workspaces)
      .values({ slug: "default", name: `${PRODUCT_NAME} Demo Workspace` })
      .returning();
  ...
  async function ensureUser(
    ...
    if (e.AUTH_DRIVER === "supabase") {
      if (password.length < 16 || password.startsWith("acqfile-"))
        throw new Error(
          `Set a unique password of at least 16 characters for ${role} before seeding Supabase Auth`,
        );
      ...
      if (u && u.id !== authUser.id)
        throw new Error(
          `Local user ${email} has a different identity; use a fresh Supabase database.`,
        );
      if (!u)
        [u] = await db
    ...
    await db
      ...
      .values({ workspaceId: ws!.id, userId: u!.id, role })
      .onConflictDoNothing();
    return u!.id;
  }
  ...
  const reviewerId = await ensureUser(
    e.DEMO_REVIEWER_EMAIL,
    e.DEMO_REVIEWER_PASSWORD,
    "Demo Reviewer",
    "reviewer",
  );
  const viewerId = await ensureUser(
    e.DEMO_VIEWER_EMAIL,
    e.DEMO_VIEWER_PASSWORD,
    "Demo Viewer",
    "viewer",
  );
