...
export function extractionConfigHash(provider = extractionProvider()) {
  return hashObject({
    ...
    prompts: [EXTRACT_PROMPT_VERSION, VERIFY_PROMPT_VERSION],
    schema: schemaHash(),
  });
}
...
async function recordCall(
  ...
  await db
    ...
    .set({
      ...
      estimatedCostUsd: sql`${schema.processingRuns.estimatedCostUsd}+${usage.cost}`,
    })
...
export async function extractSegment(
  ...
  const promptBlocks: PromptBlock[] = blocks
    .filter((b) => b.kind !== "field" || b.text)
    .map((b) => ({
      source_block_id: b.locator,
      page: b.page,
      text: b.text,
      ...(b.image_only ? { image_only: true } : {}),
    }));
  ...
  const candidates = await retry("extract", async (): Promise<Candidate[]> => {
    ...
    if (second) {
      await recordCall(context, ctx, "extract", second.usage, EXTRACT_PROMPT_VERSION);
      modelCalls++;
    }
    const out: Candidate[] = [];
    for (const def of fields) {
      ...
      const cited = field.source_block_ids
        .map((id) => blocks.find((b) => b.locator === id))
        .filter(Boolean);
      ...
      if (
        ...
        const read = parsed.identifiers.find(
          (i) =>
            i.page >= segment.pageStart &&
            i.page <= segment.pageEnd &&
            i.last_four === lastFour &&
            (def.attribute === "bank.account" ? i.kind === "account" : i.kind !== "account"),
        );
      ...
      out.push(
        scrubPayload({
          ...
          page: cited[0]?.page ?? null,
          decode_error: decoded.error,
          ...(isVision
            ? {
                second_read:
                  secondField && secondField.value !== null
                    ? decodeModelValue(def, secondField.value).value
                    : null,
              }
            : {}),
        }),
  ...
  const validations = await retry(
    "deterministic_validate",
    async (): Promise<Validation[]> =>
      validateCandidates(candidates, parsed.blocks, segment.pageStart, segment.pageEnd),
  );
  ...
  const verdicts = await retry("independent_verify", async (): Promise<Verdict[]> => {
    const items: VerifyPromptItem[] = candidates
      .filter((c) => c.method !== "acroform")
      .map((c, index) => {
        const cited = c.source_block_ids
          .map((id) => blocks.find((b) => b.locator === id))
          .filter(Boolean)
          .map((b) => ({
            source_block_id: b!.locator,
        ...
        const context_ = promptBlocks
          .filter(
            (b) => pages.has(b.page) && !cited.some((x) => x.source_block_id === b.source_block_id),
          )
          .slice(0, 12);
        return {
          index,
          attribute: c.attribute,
          candidate: JSON.stringify(c.value),
    ...
    for (let i = 0; i < items.length; i += VERIFIER_BATCH_SIZE) {
      ...
      const response = await provider.verify({
        ...
        pageStart: segment.pageStart,
        items: batch,
        pdf: batch.some((b) => b.cited_blocks.some((x) => x.image_only)) ? await pdf() : null,
      });
      ...
      for (const item of batch) {
        ...
        if (r?.corrected_value !== null && r?.corrected_value !== undefined)
          corrected = decodeModelValue(def, r.corrected_value).value;
        out.push(
          scrubPayload({
            attribute: item.attribute,
            status: (r?.status.toLowerCase() ?? "unsupported") as Verdict["status"],
            corrected_value: corrected,
            contradiction: r?.contradiction_detected ?? false,
            specificity: r?.evidence_specificity ?? 0,
            reason: r?.reason ?? "No verifier response.",
          }),
        );
      }
    }
  ...
  const scores = await retry(
    "calculate_confidence",
    async (): Promise<Scored[]> =>
      candidates.map((c) =>
        score(
          c,
          validations.find((v) => v.attribute === c.attribute)!,
          verdicts.find((v) => v.attribute === c.attribute) ?? null,
        ),
      ),
  );
  ...
  return retry("finalize_segment", async (): Promise<ExtractionSummary> => {
    ...
    return db.transaction(async (tx) => {
      ...
      for (const c of candidates) {
        const s = scores.find((x) => x.attribute === c.attribute)!,
          v = validations.find((x) => x.attribute === c.attribute)!,
          verdict = verdicts.find((x) => x.attribute === c.attribute) ?? null;
        const def = FACTS[c.attribute]!;
        if (
          def.value_type === "identifier" &&
          !(c.value && typeof c.value === "object" && "hmac" in c.value)
        ) {
          ...
          await tx
            ...
            .values({
              ...
              attribute: c.attribute,
              type: "extraction_gap",
              reason: `Only the last four digits (${(c.value as { last_four?: string } | null)?.last_four ?? "unknown"}) were read from the image; enter the identifier from the original to store it masked.`,
            })
            .onConflictDoNothing();
          continue;
        }
        if (
          !v.locators_ok &&
          s.routing === "blocked" &&
          (c.value === null || v.messages.some((m) => m.code === "type_invalid"))
        )
          continue;
        if (
          c.value === null ||
          v.messages.some((m) => m.code === "type_invalid" || m.code === "date_invalid")
        )
          continue;
        const page = c.page ?? segment.pageStart;
        const locator = {
          file: ctx.documentVersionId,
          page,
          source_block: c.source_block_ids[0] ?? `page-${page}`,
          quote: c.quote && c.quote.length ? c.quote : valueText(c.value),
          ...(c.method === "vision" ? { region: c.region ?? c.attribute, verbatim: false } : {}),
        };
        await tx.insert(schema.facts).values({
          ...
          attribute: c.attribute,
          valueJson: c.value as object,
          normalizedValueJson: normalizeFact(c.attribute, c.value) as object,
          unit: def.unit,
          period: segment.period,
          method: c.method,
          locatorJson: locator,
          confidence: String(s.confidence),
          confidenceComponents: s.components,
          validatorsPassed: v.score > 0,
          routingStatus: s.routing,
          recordVersion: maxVersion + 1,
          isCurrent: true,
          documentVersionId: ctx.documentVersionId,
          ...
          validationJson: [
            ...v.messages,
            ...s.reasons
              .filter((r) => !v.messages.some((m) => m.code === r))
              .map((code) => ({ code, level: "info" })),
          ],
          correctedValueJson:
            verdict &&
            verdict.corrected_value !== null &&
            JSON.stringify(normalizeFact(c.attribute, verdict.corrected_value)) !==
              JSON.stringify(normalizeFact(c.attribute, c.value))
              ? (verdict.corrected_value as object)
              : null,
      ...
      for (const attribute of routed.gaps)
        await tx
          ...
          .values({
            ...
            attribute,
            type: "extraction_gap",
            reason: `The extractor found no value for ${attribute}, which a rule reads from this document type.`,
          })
          .onConflictDoNothing();
      ...
      for (const c of candidates.filter(
        (x) =>
          x.method === "vision" &&
          FACTS[x.attribute]!.value_type === "identifier" &&
          segment.partyId,
      )) {
        const [party] = await tx
          .select()
          .from(schema.parties)
          .where(eq(schema.parties.id, segment.partyId!));
        ...
        if (party?.identifierLastFour && lastFour && party.identifierLastFour !== lastFour)
          await tx
            ...
            .values({
              ...
              priority: "high",
              reason: "The identifier read from the image does not match the assigned party.",
            })
            .onConflictDoNothing();
      }
      if (candidates.some((c) => c.attribute === "ownership.members"))
        await resolveOwnership(tx, dealId, segment.partyId, ctx.documentVersionId, segment.id);
