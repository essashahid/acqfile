...
export function readAcroform(
  ...
  const blocks = parsed.blocks.filter(
    (b) => b.kind === "field" && b.page >= pageStart && b.page <= pageEnd,
  );
  ...
  for (const def of fields) {
    if (def.attribute === "ownership.members" && "owners" in spec) {
      ...
      for (let i = 1; i <= spec.owners.count; i++) {
        const name = field(spec.owners.name + i),
          percent = field(spec.owners.percent + i),
          title = field(spec.owners.title + i);
    ...
    if (def.value_type === "identifier") {
      ...
      const read = parsed.identifiers.find(
        (i) => i.page === block.page && i.last_four === lastFour && i.kind !== "account",
      );
      value = read
        ? { hmac: read.hmac, last_four: read.last_four }
        : lastFour
          ? { last_four: lastFour }
          : null;
      if (!value) continue;
