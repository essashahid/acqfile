...
export function Textarea({
  ...
  return (
    <textarea ref={ref} {...props} className={cn("w-full px-2.5 py-2 leading-6", className)} />
  );
}
...
export function FilterBar({
  ...
  return (
    <div
      className={cn(
        "mb-3 rounded-[var(--r-lg)] border border-[var(--line)] bg-[var(--surface)] p-3 shadow-[var(--shadow-sm)]",
        className,
      )}
    >
      <div className="flex flex-wrap items-end gap-x-3 gap-y-3">
        ...
        {meta ? (
          <div className="ml-auto self-center text-[12px] text-[var(--muted)]">{meta}</div>
        ) : null}
      </div>
    </div>
  );
