...
export function Panel({
  ...
  return (
    <Tag
      id={id}
      className={cn(
        "panel overflow-hidden rounded-[var(--r-lg)] border border-[var(--line)] bg-[var(--surface)] shadow-[var(--shadow-sm)]",
        className,
      )}
    >
      {children}
...
export function PanelHeader({
  ...
  return (
    <div
      className={cn(
        "flex flex-wrap items-center justify-between gap-x-4 gap-y-2 border-b border-[var(--line)]",
        dense ? "bg-[var(--surface-sunken)] px-4 py-2" : "px-4 py-3",
        className,
      )}
    >
        ...
        <h3
          className={cn(
            dense
              ...
              : "text-[14px] font-semibold text-[var(--fg)]",
          )}
        >
          {title}
        </h3>
...
export function PanelFooter({
  ...
  return (
    <div
      className={cn(
        "flex flex-wrap items-center gap-3 border-t border-[var(--line)] bg-[var(--surface-sunken)] px-4 py-2.5 text-[12px] text-[var(--muted)]",
        className,
      )}
    >
      {children}
...
export function SectionTitle({
  ...
  return (
    <div
      id={id}
      className={cn("mb-2.5 flex flex-wrap items-end justify-between gap-x-4 gap-y-1", className)}
    >
      <div className="min-w-0">
        <h2 className="flex items-center gap-2 text-[15px] font-semibold tracking-[-0.01em] text-[var(--fg)]">
          <span>{title}</span>
          {count !== undefined ? (
            <span className="tnum rounded-full bg-[var(--surface-sunken)] px-2 py-0.5 text-[12px] font-medium text-[var(--muted)]">
              {count}
            </span>
          ) : null}
        </h2>
        {description ? (
          <p className="mt-0.5 text-[12.5px] text-[var(--muted)]">{description}</p>
        ) : null}
      </div>
      {actions ? <div className="flex min-w-0 flex-wrap items-center gap-2">{actions}</div> : null}
    </div>
...
export function Notice({
  ...
  return (
    <div
      ...
      className={cn(
        "flex flex-wrap items-center justify-between gap-x-4 gap-y-1.5 rounded-[var(--r-md)] border px-3.5 py-2.5 text-[13px]",
        tones[tone],
        className,
      )}
    >
