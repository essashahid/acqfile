  ...
  return (
    <div
      className={cn(
        ...
        !bare &&
          "rounded-[var(--r-lg)] border border-[var(--line)] bg-[var(--surface)] shadow-[var(--shadow-sm)]",
        className,
      )}
    >
...
export function THead({ children, sticky = false }: { children: ReactNode; sticky?: boolean }) {
  return (
    <thead
      className={cn(
        "bg-[var(--surface-sunken)] text-[11px] font-semibold uppercase tracking-[0.05em] text-[var(--muted)]",
...
export function Th({
  ...
  return (
    <th
      ...
      className={cn(
        "whitespace-nowrap border-b border-[var(--line)] px-3 py-2 font-semibold",
...
export function Tr({
  ...
  return (
    <tr
      className={cn(
        "border-b border-[var(--line)] transition-colors last:border-b-0 hover:bg-[var(--surface-hover)]",
...
export function CellStack({
  ...
  return (
    <div className={cn("min-w-0", className)}>
      ...
      {secondary ? (
        <div className="mt-0.5 truncate text-[12px] text-[var(--muted)]">{secondary}</div>
      ) : null}
    </div>
  );
}
...
export const rowLink =
  "font-medium text-[var(--fg)] underline decoration-[var(--line-strong)] decoration-1 underline-offset-2 transition-colors hover:text-[var(--accent)] hover:decoration-[var(--accent)]";

/** Standalone inline link (outside tables). */
export const inlineLink =
  "text-[var(--accent)] underline decoration-[var(--accent-border)] underline-offset-2 transition-colors hover:decoration-[var(--accent)]";
