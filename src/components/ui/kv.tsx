    ...
    <dl className={cn("text-[13px]", className)}>
      {items.map((item, i) => (
        <div
          ...
          title={item.title}
          className={cn(
            "gap-x-4 gap-y-0.5 px-4 py-2.5 sm:flex",
            dividers && "border-b border-[var(--line)] last:border-b-0",
          )}
        >
          <dt
            className="shrink-0 text-[12px] font-medium text-[var(--muted)] sm:pt-px"
            style={{ width: labelWidth }}
          >
            {item.label}
          </dt>
          <dd className="mt-0.5 min-w-0 flex-1 break-words text-[var(--fg)] sm:mt-0">
            {item.value}
          </dd>
        </div>
      ))}
...
export function MetaRow({
  ...
  items: { label: ReactNode; value: ReactNode; title?: string }[];
  className?: string;
}) {
  return (
    <div
      className={cn(
        "flex flex-wrap items-center gap-x-4 gap-y-1 text-[12.5px] text-[var(--muted)]",
        className,
      )}
    >
      {items.map((item, i) => (
