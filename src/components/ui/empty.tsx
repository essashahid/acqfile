...
export function EmptyState({
  ...
  return (
    <div
      className={cn(
        "flex flex-col items-center justify-center rounded-[var(--r-lg)] border border-dashed border-[var(--line-strong)] bg-[var(--surface)] text-center",
    ...
    >
      ...
      {children ? (
        <div className="mt-1 max-w-md text-[13px] leading-5 text-[var(--muted)]">{children}</div>
      ) : null}
      {action ? (
...
export function EmptyLine({
  ...
  return (
    <div
      className={cn(
        "rounded-[var(--r-lg)] border border-dashed border-[var(--line)] bg-[var(--surface)] px-3.5 py-2.5 text-[13px]",
        tone === "ok" ? "text-[var(--muted)]" : "text-[var(--muted)]",
        className,
      )}
    >
      {children}
