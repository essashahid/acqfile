  ...
  const body = (
    <>
      <div className="flex items-start justify-between gap-2">
        <span className="text-[11px] font-semibold uppercase tracking-[0.05em] text-[var(--muted)]">
          {label}
        </span>
        {target ? (
          <span
            ...
            className={cn(
              ...
              target.met
                ...
                : "border-[var(--bad-border)] bg-[var(--bad-soft)] text-[var(--bad)]",
            )}
          >
      ...
      <div
        className={cn(
          ...
          size === "lg" ? "text-[28px] leading-9" : "text-[22px] leading-7",
          VALUE_TONE[tone],
        )}
      >
        {value}
...
export function MetricStrip({
  ...
  items: { label: ReactNode; value: ReactNode; tone?: Tone; title?: string }[];
  className?: string;
}) {
  return (
    <dl
      className={cn(
        "grid grid-cols-2 divide-[var(--line)] sm:grid-cols-3 sm:divide-x lg:grid-cols-6",
        className,
      )}
    >
      {items.map((item, i) => (
        <div
          key={i}
          title={item.title}
          className="min-w-0 px-3.5 py-3 first:pl-4 max-sm:border-b max-sm:border-[var(--line)] max-sm:odd:border-r"
        >
          <dt className="truncate text-[11px] font-semibold uppercase tracking-[0.05em] text-[var(--muted)]">
            {item.label}
          </dt>
          <dd
            className={cn(
              "tnum mt-1 text-[16px] font-semibold",
