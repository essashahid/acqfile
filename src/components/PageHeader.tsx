    ...
    <header className={cn("mb-5", className)}>
      {breadcrumbs?.length ? (
        <nav
          ...
          className="mb-1.5 flex flex-wrap items-center gap-1 text-[12.5px] text-[var(--muted)]"
        >
          {breadcrumbs.map((c, i) => (
      ...
      <div className="flex flex-wrap items-start justify-between gap-x-6 gap-y-3">
        <div className="flex min-w-[min(100%,17rem)] flex-1 gap-3">
          {section ? (
            <SectionIcon section={section} size="md" className="mt-0.5 hidden sm:grid" />
          ) : null}
          <div className="min-w-0 flex-1">
            <h1 className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-[24px] font-semibold leading-8 tracking-[-0.02em]">
              {title}
            </h1>
            {subtitle ? (
              <p className="mt-1 max-w-3xl text-[13.5px] leading-5 text-[var(--muted)]">
                {subtitle}
              </p>
            ) : null}
