...
const result = exportReview(
  PACK_VERSIONS.flatMap((v) => [loadPack(v), loadPack(v, "sample-lender-a")]),
);
fs.writeFileSync("docs/RULEPACK_REVIEW.xlsx", result.xlsx);
fs.writeFileSync("docs/RULEPACK_REVIEW.html", result.html);
console.log(
  "PASS: docs/RULEPACK_REVIEW.xlsx and docs/RULEPACK_REVIEW.html (four resolved configurations)",
);
