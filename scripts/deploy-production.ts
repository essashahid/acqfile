...
function main() {
  ...
  if (!fs.existsSync(file))
    throw new Error(`Create ${file} from .env.example and fill the production credentials.`);
  ...
  if (missing.length)
    throw new Error(`Fill ${missing.join(", ")} in ${file}. No deployment changes made.`);
  if (config.AUTH_SECRET!.length < 48)
    throw new Error("AUTH_SECRET must be a random value of at least 48 characters.");
  ...
  if (
    ...
    throw new Error("Production needs a hosted direct PostgreSQL connection.");
  if (
    config.AUTH_DRIVER !== "database" ||
    config.STORAGE_DRIVER !== "blob" ||
    config.JOB_DRIVER !== "inngest"
  )
    throw new Error("Production drivers must be database/blob/inngest.");
  for (const key of required.filter((key) => key.endsWith("PASSWORD")))
    if (config[key]!.length < 16) throw new Error(`${key} must have at least 16 characters.`);
  ...
  console.log(
    "Verify the deployed /api/inngest app is synced in Inngest, then exercise a hosted upload and retry.",
  );
}
try {
  main();
