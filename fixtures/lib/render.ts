...
export async function textPdf(p:Plan,docs:Doc[],acro=false){if(docs.length===1&&officialForm(docs[0]!.type))return officialPdf(p,docs[0]!);const pdf=await PDFDocument.create();pdf.setCreationDate(FIXED_DATE);pdf.setModificationDate(FIXED_DATE);pdf.setCreator('AcqFile synthetic fixtures');pdf.setProducer('AcqFile synthetic fixtures');const font=await pdf.embedFont(StandardFonts.Helvetica);const bold=await pdf.embedFont(StandardFonts.HelveticaBold);
 for(const d of docs){const page=pdf.addPage([612,792]);page.drawRectangle({x:30,y:714,width:552,height:50,color:rgb(.93,.95,.96)});const lines=content(p,d);let y=748;for(const [lineIndex,line] of lines.entries()){const face=lineIndex<2?bold:font;const size=Math.min(lineIndex===0?16:lineIndex===1?10:10,540/Math.max(1,face.widthOfTextAtSize(line,1)));page.drawText(line,{x:36,y,size,font:face,color:rgb(.12,.16,.2)});
...
export async function verifyAcroform(bytes:Buffer,docs:Doc[]){if(docs.length===1&&officialForm(docs[0]!.type))return verifyOfficial(bytes,docs[0]!);const pdf=await PDFDocument.load(bytes);for(const d of docs)for(const [a,v] of Object.entries(d.facts)){const field=pdf.getForm().getField(`${d.id}.${a}`);if(!(field instanceof PDFTextField)||field.getText()!==display(v))throw Error(`AcroForm readback mismatch ${d.id}/${a}`);}return pdf.getForm().getFields().length;}
