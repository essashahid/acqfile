...
export function lintSynthetic(text:string){assert.ok(!/\b(?:eligible|ineligible|qualifies|approved|compliant|meets SBA requirements)\b/i.test(text),'Banned terminology');for(const m of text.matchAll(/\b(\d{3})-(\d{2})-(\d{4})\b/g))assert.ok(Number(m[1])>=900,`SSN outside synthetic range: ${m[0]}`);for(const m of text.matchAll(/\b(\d{2})-(\d{7})\b/g))assert.equal(m[1],'00','EIN outside synthetic range');for(const m of text.matchAll(/\(?\b(\d{3})\)?[ .-]+(\d{3})[ .-]+(\d{4})\b/g))assert.ok(m[2]==='555'&&m[3]!.startsWith('01'),`Phone outside synthetic range: ${m[0]}`);for(const m of text.matchAll(/[\w.+-]+@([\w.-]+\.[A-Za-z]{2,})/g))assert.equal(m[1],'example.com','Non-example email');}
export function lintRepositoryNames(){const files=execFileSync('git',['ls-files','--cached','--others','--exclude-standard','-z'],{encoding:'utf8'}).split('\0').filter(Boolean);const old=[['North','field'].join(''),['Harbor','view'].join('')];for(const file of files){if(!fs.existsSync(file)||file==='docs/SPEC.md'||file==='docs/SPEC_AMENDMENTS.md'||/^docs\/PHASE_\d+\.md$/.test(file)||file.startsWith('fixtures/legacy/'))continue;if(/\.(pdf|xlsx|docx|zip|png|jpg|tgz)$/.test(file))continue;const text=fs.readFileSync(file,'utf8');for(const name of old)assert.ok(!text.toLowerCase().includes(name.toLowerCase()),`${file}: forbidden former name`);}}
