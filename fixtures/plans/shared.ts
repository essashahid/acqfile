...
export type Format = "text_pdf" | "acroform_pdf" | "docx" | "xlsx" | "scan_pdf" | "bundle_pdf" | "protected_pdf";
export type Doc = { id:string; type:DocumentType; party:string; period?:string; facts:Record<string,unknown>; metadata:Record<string,unknown>; batch:number; format:Format; path:string; group?:string; duplicate_of?:string; supersedes?:string; unreadable?:boolean; tags:number[]; notes:string[] };
...
export function base(id:"deal-a"|"deal-b"|"deal-c"):Plan {
 ...
 const target=["Varnholt Climate Services LLC","Quenby Grounds Management LLC","Ostrelyva Fitness LLC"][n]!;
 const buyer=["Varnholt Acquisition LLC","Quenby Acquisition LLC","Ostrelyva Acquisition LLC"][n]!;
 const price=[2400000,3600000,850000][n]!;
 const model:Model={price,project:price+100000,loan:0,cash:150000,note:n===0?240000:n===1?120000:0,gift:n===2?50000:0,investor:n===1?90000:0,years:{"2023":{revenue:1200000,income:120000},"2024":{revenue:1300000,income:130000},"2025":{revenue:1400000,income:140000}},assets:500000,liabilities:100000,equity:400000,personal:{cash:180000,assets:500000,liabilities:100000,netWorth:400000}};
 model.loan=model.project-model.cash-model.note-model.gift-model.investor;
 const b=structuredClone(baseline);
 const replacements:Record<string,string>={"Varnholt Climate Services, LLC":target,"Varnholt Acquisition LLC":buyer,"Kiel McDermott":person(20260915+n*10)};
 function replace(v:unknown):unknown {if(typeof v==="string")return replacements[v]??v;if(Array.isArray(v))return v.map(replace);if(v&&typeof v==="object")return Object.fromEntries(Object.entries(v).map(([k,v])=>[k,replace(v)]));return v;}
 ...
 for(const d of documents){
  ...
  if(d.type==="BANK_STATEMENT")Object.assign(d.facts,{"bank.ending_balance":model.personal.cash,"bank.institution":"Zelmivar Bank"});
  if(d.type==="SBA_413")Object.assign(d.facts,{"pfs.cash":model.personal.cash,"pfs.total_assets":model.personal.assets,"pfs.total_liabilities":model.personal.liabilities,"pfs.net_worth":model.personal.netWorth});
  if(d.type==="TAX_BUSINESS")Object.assign(d.facts,{"tax.net_income":model.years[d.period!]!.income,"tax.year":Number(d.period),"tax.form_type":"1120S"});
  if(d.type==="FIN_YEAR_END")d.facts["financial.net_income"]=model.years[d.period!]!.income;
  if(d.type==="TAX_PERSONAL")Object.assign(d.facts,{"tax.year":Number(d.period),"party.identifier":fakeId("900-12-3456")});
 }
 const sources=[{label:"Senior loan",amount:model.loan},{label:"Cash injection",amount:model.cash},...(model.note?[{label:"Seller note",amount:model.note}]:[]),...(model.gift?[{label:"Gift",amount:model.gift}]:[]),...(model.investor?[{label:"Minority investor",amount:model.investor}]:[])];
 Object.assign(doc(plan,"funding").facts,{"funding.sources":sources,"funding.uses":[{label:"Purchase",amount:price},{label:"Working capital",amount:100000}],"funding.sources_total":model.project,"funding.uses_total":model.project});
 if(model.note){
  for(const id of ["loi","purchase","funding"])Object.assign(doc(plan,id).facts,{"deal.seller_note_amount":model.note,"deal.seller_note_terms":"Full lifetime standby"});
  add(plan,"note","SELLER_NOTE","target",{"note.principal":model.note,"note.term_months":120,"note.full_standby":true,"deal.seller_note_terms":"Full lifetime standby"});plan.tracking.push("TXN-05");
  if(Array.isArray(profile.equity_sources))profile.equity_sources.push({id:"seller-note",party:"target",kind:"seller_standby_note",amount:model.note,source_account_last_four:"9011"});
 }
 return plan;
}
export function doc(p:Plan,id:string){const d=p.documents.find(d=>d.id===id);if(!d)throw Error(`Unknown ${id}`);return d;}
export function add(p:Plan,id:string,type:DocumentType,party:string,facts:Record<string,unknown>={},period?:string){const d:Doc={id,type,party,facts,period,metadata:{signed:true,dated:true,signature_date:"2026-08-31",document_date:"2026-08-31",form_revision:null,account_last_four:null},batch:1,format:"text_pdf",path:"",tags:[],notes:[]};p.documents.push(d);return d;}
export function party(p:Plan,id:string,name:string,roles:Party["roles"],kind:Party["kind"]="individual"){p.parties.push({id,kind,roles,legal_name:name,affiliates:[],jointly_held_assets:"no",name_variants:[],identifier:null});}
export function owner(p:Plan,id:string,percent:number,owned="buyer"){p.ownership.push({owner_party_id:id,owned_party_id:owned,percent,stage:"post_closing",origin:"declared"});}
export function setOwners(p:Plan,owners:{name:string;percent:number}[]){for(const id of ["1919","operating","owners"])doc(p,id).facts["ownership.members"]=structuredClone(owners);}
export function guarantor(p:Plan,id:string,seed:number){
 ...
 for(const d of [...p.documents].filter(d=>d.party==="alex")){
  const copy=structuredClone(d);copy.id=`${id}-${d.id}`;copy.party=id;
  if(copy.type==="BANK_STATEMENT"){copy.metadata.account_last_four="6543";copy.facts["party.legal_name"]=person(seed);}
...
export function checklist(p:Plan,guarantors:string[],owners:string[],sources:{id:string;kind:string}[],affiliate?:string){
 ...
 row(p,["TXN-06","RE-01","RE-02","RE-03"],"deal","not_applicable");row(p,["TXN-07"],"none","not_applicable");
 if(p.id!=="deal-a")row(p,["TXN-09"],"deal",p.id==="deal-b"?"satisfied":"not_applicable");
 row(p,["ENT-01","ENT-02","ENT-03","ENT-04","ENT-05","ENT-06","ENT-07a","ENT-07b"],"buyer");
 for(const g of guarantors){row(p,["GUA-01","GUA-03","GUA-04","GUA-06"],g);row(p,["GUA-02"],g,"satisfied",YEARS);}
 for(const o of owners)row(p,["GUA-05"],o);
 for(const s of sources){row(p,["GUA-07"],s.id,s.kind==="cash"?"satisfied":"not_applicable",MONTHS);row(p,["GUA-08a","GUA-08b","GUA-08c"],s.id,s.kind==="gift"?"satisfied":"not_applicable");}
 if(affiliate){row(p,["GUA-09a"],affiliate,"satisfied",YEARS);row(p,["GUA-09b","GUA-09c"],affiliate);}else row(p,["GUA-09a","GUA-09b","GUA-09c"],"none","not_applicable");
 ...
 if(p.id==="deal-b"){row(p,["TXN-10a"],"deal");row(p,["TGT-11"],"target");row(p,["TXN-10b","TXN-10c"],"deal","not_applicable");row(p,["TGT-12a","TGT-12b","TGT-12c"],"target","not_applicable");}
}
export function status(p:Plan,rule:string,scope:string,value:string,period:string|null=null){const r=p.batches[0]!.checklist.find(r=>key(r.item_id,r.scope_key,r.period)===key(rule,scope,period));if(!r)throw Error(`Missing authored row ${rule}/${scope}/${period}`);r.status=value;}
export function layout(p:Plan,targetFiles:number){
 ...
 while(fileGroups()>targetFiles){
  ...
  const group=`forwarded-packet-${++serial}`;for(const d of candidates.slice(0,2))d.group=group;
 }
 for(const d of p.documents){const group=d.group??d.id;const grouped=p.documents.filter(x=>(x.group??x.id)===group);if(grouped.length>1)d.format=p.id==="deal-c"?"scan_pdf":"bundle_pdf";
