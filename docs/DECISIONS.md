...
13. Northfield requires TXN-10a non-compete and TGT-11 add-back schedule, as selected by the user. Preserve TXN-10b/c and TGT-12a/b/c as `required: false` definitions so later lender configuration can enable them without code changes. Small choices of this size are decided and documented without asking.
...
22. Compare legal-name suffixes, punctuation and spacing deterministically; this includes Harborview versus Harbor View. These comparisons do not merge parties. Use USD 1 for arithmetic rounding, USD 0 for purchase-price agreement, 0.01 percentage points for ownership totals, and 1% for tax/revenue differences. Every flag remains a preparation aid.
