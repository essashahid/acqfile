...
test('rule viewer compares packs, shows sources and applies Northfield parameters',async({page})=>{
 ...
 await page.locator('select[name="overlay"]').selectOption('northfield-bank');await page.getByRole('button',{name:'View comparison'}).click();
 await expect(page.getByRole('region',{name:'Pack parameters'})).toContainText('northfield-bank');
 ...
 await expect(page.getByRole('region',{name:'Pack parameters'})).toContainText('NF_{party}');
