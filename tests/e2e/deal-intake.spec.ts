...
test("Deal A: import, batch intake, bundle review, fact review, evaluation counts and corrected version", async ({
  ...
  await expect(page.getByRole("heading", { name: "Filed documents", exact: true })).toBeVisible();
  await expect(
  ...
  await page
    ...
    .first()
    .click();
  await expect(page.getByRole("heading", { name: "Segments and filing" })).toBeVisible();
  ...
  await page
    ...
    .fill("Confirmed the logical document page ranges against the supplied pages.");
  ...
  await page
    ...
    .first()
    .click();
  await page.getByLabel("Segment 1 type", { exact: true }).selectOption("FORMATION_DOC");
  await page
    .getByLabel("Review note", { exact: true })
    .fill("Manually indexed as formation document; encrypted copy remains unreadable.");
  ...
  await page.getByRole("button", { name: "Upload batch" }).click();
  await expect(page.getByRole("status")).toContainText("Batch 2: 3 files received", {
    timeout: 120000,
  });
  await expect(
  ...
  expect(
    missingBefore -
      (await count("findings-missing")) +
      (incompleteBefore - (await count("findings-incomplete"))),
  ).toBe(3);
  await page.getByRole("heading", { name: "Documents", exact: true }).scrollIntoViewIfNeeded();
