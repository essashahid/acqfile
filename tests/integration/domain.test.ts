...
let seed: Awaited<ReturnType<typeof seeded>>, deal: string, otherDeal: string, party: string, version: string, segment: string;
beforeAll(async()=>{
...
describe('additive domain storage',()=>{
 ...
 it('enforces same-deal references for segments, facts and ownership',async()=>{
  const sql=getSql();
  await expect(sql`insert into segments(deal_id,document_version_id,metadata_locator,page_start,page_end,doc_type,classification_method,classification_confidence,status) values (${otherDeal},${version},'{}',1,2,'EIN_LETTER','manual',1,'confirmed')`).rejects.toThrow('another deal');
  await expect(sql`update documents set deal_id=${otherDeal} where id=(select document_id from document_versions where id=${version})`).rejects.toThrow('Cannot reassign');
  await expect(sql`insert into facts(deal_id,segment_id,attribute,value_json,normalized_value_json,unit,method,locator_json,confidence,confidence_components,validators_passed,routing_status,record_version) values (${otherDeal},${segment},'party.legal_name','"Synthetic"','"Synthetic"','text','text','{"file":"test.pdf","page":1,"source_block":"p1","quote":"Synthetic"}',1,'{}',true,'accepted',1)`).rejects.toThrow('foreign key');
  await expect(sql`insert into ownership_links(deal_id,owner_party_id,owned_party_id,percent,stage,origin) values (${otherDeal},${party},${party},100,'post_closing','declared')`).rejects.toThrow();
 });
 it('stores masked identifiers and requires manual audit evidence',async()=>{
  const sql=getSql();const identifier=JSON.stringify({hmac:'a'.repeat(64),last_four:'1234'}),locator=JSON.stringify({file:'domain.pdf',page:1,source_block:'1',quote:'Identifier ending 1234'});
  const events=await sql`insert into events(deal_id,actor_id,action,entity_type,entity_id,masked_after) values (${deal},${seed.adminId},'manual_fact','segment',${segment},${identifier}::jsonb) returning id`;
  const event=events[0]!.id;
  const insert=(value:string,audit:string|null)=>sql`insert into facts(deal_id,segment_id,subject_party_id,attribute,value_json,normalized_value_json,unit,method,locator_json,confidence,confidence_components,validators_passed,routing_status,actor_id,audit_event_id,record_version) values (${deal},${segment},${party},'party.identifier',${value}::jsonb,${value}::jsonb,'masked_identifier','manual',${locator}::jsonb,1,'{}',true,'accepted',${seed.adminId},${audit},1)`;
  await expect(insert(identifier,null)).rejects.toThrow();await expect(insert(JSON.stringify({clear:'123456789'}),event)).rejects.toThrow();await expect(insert(identifier,event)).resolves.toBeDefined();
  await expect(sql`update events set action='edited' where id=${event}`).rejects.toThrow('Immutable');await expect(sql`delete from events where id=${event}`).rejects.toThrow('Immutable');
 });
 it('rejects invalid page ranges and confidence',async()=>{
  const sql=getSql();await expect(sql`insert into segments(deal_id,document_version_id,metadata_locator,page_start,page_end,doc_type,classification_method,classification_confidence,status) values (${deal},${version},'{}',5,2,'EIN_LETTER','manual',1.2,'confirmed')`).rejects.toThrow();
 });
});
