...
it("Phase 4 Step B: evaluation before and after simulated review equals the authored expectations", async () => {
  ...
  for (const code of ["deal-a", "deal-b", "deal-c"]) {
    ...
    const dealId = await saveDeal(ctx, {
      ...dealJson,
      code: `E-${code}`,
      name: dealJson.parties.find((p: { roles: string[] }) => p.roles.includes("seller_entity"))
        .legal_name,
    });
    ...
    const authored = JSON.parse(
      fs.readFileSync(`fixtures/deals/${code}/truth/batch-1/engine_input.json`, "utf8"),
    ) as {
      tracking: { rule_id: string; scope_key: string; state: "received" }[];
    ...
    for (const t of authored.tracking) {
      await unlimited();
      await recordAttestation(ctx, dealId, {
        kind: "tracking",
        rule_id: t.rule_id,
        scope_key: internal(t.scope_key),
        state: t.state,
        note: "Receipt recorded for fixture",
      });
    }
    for (const c of authored.manual_confirmations) {
      await unlimited();
      await recordAttestation(ctx, dealId, {
        kind: "manual_confirmation",
        rule_id: c.rule_id,
        scope_key: internal(c.scope_key),
        key: c.key,
    ...
    for (let batch = 1; batch <= (code === "deal-a" ? 2 : 1); batch++) {
      const expectedRows = JSON.parse(
        fs.readFileSync(
          `fixtures/deals/${code}/truth/batch-${batch}/expected_checklist.json`,
          "utf8",
        ),
      ) as Row[];
      const expectedFindings = (
        JSON.parse(
          fs.readFileSync(
            `fixtures/deals/${code}/truth/batch-${batch}/expected_findings.json`,
            "utf8",
          ),
        ) as (ExpectedFinding & { finding_key: string })[]
      ).map(({ rule_id, scope_key, period, type, severity }) => ({
        rule_id,
        scope_key,
        period,
        type,
      ...
      const actualRows = async () => {
        ...
        return {
          rows: rows.map((r) => ({
            item_id: r.itemId,
            scope_key: external(r.scopeKey),
            period: r.period || null,
            status: r.status,
          })),
          findings: findings.map((f) => ({
            rule_id: f.ruleId,
            scope_key: external(f.scopeKey),
            period: f.period,
            type: f.type,
            severity: f.severity,
          })),
      ...
      const expectedByKey = new Map(
        expectedRows.map((r) => [`${r.item_id}|${r.scope_key}|${r.period}`, r.status]),
      );
      const falseSatisfied = before.rows.filter(
        (r) =>
          r.status === "satisfied" &&
          expectedByKey.get(`${r.item_id}|${r.scope_key}|${r.period}`) !== "satisfied",
      );
      ...
      for (const f of pending.filter((f) =>
        ...
        const version = truth.find(
          (d) =>
            ...
            d.hash ===
              (upload.rows.find((r) => r.documentVersionId === segment!.documentVersionId)
                ?.contentHash ?? truth.find((t) => t.segments.length && t.file.includes(""))?.hash),
        );
        const truthSegment =
          version?.segments.find((s) => s.page_start === segment!.pageStart) ??
          truth.flatMap((d) => d.segments).find(() => false);
        const truthFact = truthSegment
          ? version!.facts.find(
              (x) => x.segment_id === truthSegment.id && x.attribute === f.attribute,
            )
          : undefined;
        await unlimited();
        if (!truthFact) {
          await reviewFact(ctx, dealId, {
            ...
            action: "reject",
            comment: "Not stated in the document.",
          });
          rejected++;
          continue;
        }
        if (same(f.valueJson, truthFact.value)) {
          await reviewFact(ctx, dealId, {
            fact_id: f.id,
            expected_record_version: f.recordVersion,
            action: "edit_accept",
      ...
      for (const gap of await db
        ...
        const [segment] = await db
          .select()
          .from(schema.segments)
          .where(eq(schema.segments.id, gap.segmentId!));
        const hash = upload.rows.find(
          (r) => r.documentVersionId === segment!.documentVersionId,
        )?.contentHash;
        const version = truth.find((d) => d.hash === hash);
        const truthSegment = version?.segments.find((s) => s.page_start === segment!.pageStart);
        const truthFact = truthSegment
          ? version!.facts.find(
              (x) => x.segment_id === truthSegment.id && x.attribute === gap.attribute,
            )
          : undefined;
        await unlimited();
        if (!truthFact) {
          await resolveGap(ctx, dealId, {
            review_id: gap.id,
            action: "dismiss",
            comment: "The document does not state it.",
          });
          dismissed++;
          continue;
        }
        const value =
          truthFact.attribute.endsWith("identifier") || truthFact.attribute === "bank.account"
            ? (truthFact.value as { hmac: string })
            : truthFact.value;
        await resolveGap(ctx, dealId, {
          review_id: gap.id,
          action: "enter",
      ...
      report.push({
        deal: code,
        batch,
        before_false_satisfied: falseSatisfied.length,
        review: { accepted, edited, rejected, entered, dismissed },
        counts,
      });
    }
    if (code === "deal-b") {
      ...
      await unlimited();
      const v2 = await reviewFact(ctx, dealId, {
        fact_id: fact!.id,
