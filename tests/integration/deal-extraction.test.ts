...
type Truth = ReturnType<typeof documents>[number] & { faults: { id: string; document: string; attribute: string; kind: string; expected: "review" | "blocked" }[] };
...
it("Phase 4 Step A: facts by method, provenance, planted faults, vision routing and resumability", async () => {
  ...
  for (const code of ["deal-a", "deal-b", "deal-c"]) {
    ...
    const dealId = await saveDeal(ctx, { ...draft, code: `X-${code}`, name: draft.parties.find((p: { roles: string[] }) => p.roles.includes("seller_entity")).legal_name });
    for (let batch = 1; batch <= (code === "deal-a" ? 2 : 1); batch++) {
      ...
      if (code === "deal-b" && batch === 1) {
        ...
        const target = upload.rows.find((r) => !r.duplicate && /loi|note|debt/i.test(r.originalPath) && r.originalPath.toLowerCase().endsWith("pdf"))!;
        injected = { dealId, versionId: target.documentVersionId, runId: upload.runId };
        await expect(processDealVersion(ctx, dealId, target.documentVersionId, upload.runId, { injectFailure: { step: "extract", attempts: 4 }, sleep: async () => {} })).rejects.toThrow();
        await processDealVersion(ctx, dealId, target.documentVersionId, upload.runId, { injectFailure: { step: "independent_verify", attempts: 1 }, sleep: async () => {} });
      }
      expect(await processDealRun(ctx, dealId, upload.runId, { sleep: async () => {} })).toEqual({ completed: upload.rows.filter((r) => !r.duplicate).length, failed: 0 });
      ...
      const counts = { deal: code, batch, facts: 0, rule_feeding: { acroform: [0, 0], text: [0, 0], vision: [0, 0] } as Record<string, [number, number]>, provenance: [0, 0] as [number, number], by_method: {} as Record<string, number>, routing: {} as Record<string, number>, vision_auto_accepted: 0, review_items: 0, gaps: 0 };
      for (const arrival of upload.rows.filter((r) => !r.duplicate)) {
        const expected = truth.find((d) => d.file === `incoming/batch-${batch}/${arrival.originalPath}`)!;
        if (!expected.segments.length) continue;
        const parsed = await parseArrival(fs.readFileSync(`fixtures/deals/${code}/${expected.file}`), FIXTURE_HMAC_KEY);
        const segments = await db.select().from(schema.segments).where(and(eq(schema.segments.documentVersionId, arrival.documentVersionId), eq(schema.segments.isCurrent, true))).orderBy(asc(schema.segments.pageStart));
        for (const truthSegment of expected.segments) {
          ...
          const facts = await db.select().from(schema.facts).where(and(eq(schema.facts.segmentId, segment.id), eq(schema.facts.isCurrent, true)));
          ...
          for (const f of facts) {
            ...
            if (f.method === "vision" && f.routingStatus === "auto_accepted") counts.vision_auto_accepted++;
            if (f.method !== "vision") {
              const locator = f.locatorJson as { source_block: string; quote: string; page: number };
              const block = parsed.blocks.find((b) => b.locator === locator.source_block);
              const valid = !!block && block.page >= segment.pageStart && block.page <= segment.pageEnd && normalizeText(block.text).includes(normalizeText(locator.quote));
              counts.provenance[0] += Number(valid); counts.provenance[1]++;
            }
          }
          for (const t of expected.facts.filter((x) => x.segment_id === truthSegment.id && consumed.has(x.attribute))) {
            ...
            else misses.push({ deal: code, batch, segment: truthSegment.id, attribute: t.attribute, method, actual: actual ? { value: actual.valueJson, routing: actual.routingStatus } : null });
          }
          for (const fault of expected.faults.filter((f) => f.document === truthSegment.id)) {
            ...
            const gaps = await db.select().from(schema.intakeReviews).where(and(eq(schema.intakeReviews.segmentId, segment.id), eq(schema.intakeReviews.type, "extraction_gap")));
            const routed = fault.kind === "missing_value" ? (!actual && gaps.some((g) => g.attribute === fault.attribute) ? "review" : "none") : (actual?.routingStatus ?? "none");
            faultLog.push({ deal: code, fault: fault.id, kind: fault.kind, expected: fault.expected, actual: routed, confidence: actual?.confidence ?? null });
      ...
      counts.review_items = pending.filter((f) => f.routingStatus === "review" || f.routingStatus === "blocked").length + reviews.length;
      counts.gaps = reviews.filter((r) => r.type === "extraction_gap").length;
      for (const method of ["acroform", "text"]) { const [ok, total] = counts.rule_feeding[method]!; if (total) expect(ok / total, `${code}/${batch} ${method}`).toBeGreaterThanOrEqual(0.95); }
      expect(counts.provenance[0] / counts.provenance[1], `${code}/${batch} provenance`).toBeGreaterThanOrEqual(0.98);
  ...
  const steps = await db.select().from(schema.runSteps).where(eq(schema.runSteps.documentVersionId, injected!.versionId));
  ...
  expect(steps.filter((s) => s.stepName === "independent_verify").every((s) => s.status === "succeeded" && s.attemptCount === 2)).toBe(true);
  const calls = async () => db.select().from(schema.llmCalls).where(eq(schema.llmCalls.documentVersionId, injected!.versionId));
  const before = await calls();
  expect(before.filter((c) => c.purpose === "extract")).toHaveLength(extractSteps.length);
  expect(before.filter((c) => c.purpose === "verify")).toHaveLength(steps.filter((s) => s.stepName === "independent_verify").length);
  await processDealVersion(ctx, injected!.dealId, injected!.versionId, injected!.runId, { sleep: async () => {} });
  ...
  for (const table of ["facts", "intake_reviews", "events", "run_steps", "run_events", "llm_calls"]) {
    const rows = await getSql().unsafe(`select to_jsonb(t)::text as row from ${table} t`);
    for (const r of rows as unknown as { row: string }[]) expect(r.row.replace(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}|[0-9a-f]{32,}/g, ""), table).not.toMatch(pattern);
  }
  fs.writeFileSync("/tmp/acqfile-phase4-step-a-proof.json", JSON.stringify({ report, faults: faultLog, misses }, null, 2));
}, 600000);
