...
import { fixtureInput } from "./engine-fixtures";
const run = (input: ReturnType<typeof fixtureInput>) =>
  evaluateDeal(input, loadPack("sop-50-10-8"));
it("absence short circuits signature and page checks; proposed evidence waits", () => {
  ...
  pack.items
    .find((r) => r.id === "ENT-01")!
    .checks.push({ type: "page_completeness", message: "All pages present" });
  const out = evaluateDeal(input, pack);
  expect(out.checklist.find((r) => r.item_id === "ENT-01")).toMatchObject({
    status: "missing",
    reasons: [],
  });
  input.segments.push({
...
it("unsigned with a null date is incomplete; reports failed and unknown checks", () => {
  const input = fixtureInput();
  Object.assign(input.segments.find((s) => s.id === "1919")!, {
    signed: false,
    signature_date: null,
    form_revision: null,
  });
...
it("missing inventory outranks a waiver", () => {
  const input = fixtureInput();
  input.evidence_inventory.segment_ids.push("absent");
  input.waivers.push({
    rule_id: "ENT-02",
    scope_key: "buyer",
    period: null,
