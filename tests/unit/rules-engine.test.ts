...
import { actualRows, expectedRows, fixtureInput, readFixture, applyCase, type Case } from "./engine-fixtures";
describe("hand-authored engine proof", () => {
 it("clean: all applicable rows satisfied, zero findings", () => {
  ...
  expect(actualRows(result.checklist)).toEqual(expectedRows(readFixture("clean").expected.checklist));
