...
vi.mock("@/lib/env", () => ({
  env: () => ({ PUBLIC_DEMO_MODE: true, DEMO_MUTATIONS_ENABLED: false }),
...
const context = (role: WorkspaceRole, isPublic = false): SessionContext => ({
  user: { id: "u", email: "test@example.test", displayName: "Test" },
  workspace: { workspaceId: "w", slug: "demo", name: "Demo", role },
  isPublic,
});
describe("public demo authorization", () => {
  ...
  it("rejects job mutations before database writes while Inngest is missing", async () => {
    await expect(assertMutation(context("admin"), "upload")).rejects.toThrow(
      "Background processing is not configured",
    );
  });
